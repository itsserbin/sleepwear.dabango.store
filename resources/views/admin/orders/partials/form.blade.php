<div class="form-group">
    ...
    <select class="form-control" id="status" name="status">
        ...
        <option value="Передан поставщику" @if ($order->status == 'Передан поставщику') selected="" @endif>Передан поставщику</option>
        ...
        <div class="col-12 col-md-3">
            <div class="form-group my-3">
                ...
                <input type="text" class="form-control" id="product_sizes" name="product_sizes"
                       value="@if(isset($order->sizes)) @foreach($order->sizes as $size){{$size}} @endforeach @else Размер не выбран @endif" disabled>
            </div>
        </div>
        ...
        <div class="col-12 col-md-3">
            <div class="form-group my-3">
                ...
                <input type="text" class="form-control" id="product_colors" name="product_colors"
                       value="@if(isset($order->colors)) @foreach($order->colors as $color){{$color}} @endforeach @else Цвет не выбран @endif" disabled>
            </div>
        </div>
{{--    </div>--}}
